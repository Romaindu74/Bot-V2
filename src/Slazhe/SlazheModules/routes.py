...
class SlashCommand(HTTPClient):
    ...
    def request(self, route, **params) -> Response:
        return super().request(route, base = self.BASE, **params)
    ...
    def bulk_upsert_global_commands(
        self, application_id: int, payload: list[dict[str, Any]]
    ) -> Response:
        r = Route('PUT', '/{application_id}/commands', application_id=application_id)
        return self.request(r, json = payload)

    def bulk_upsert_guild_commands(self, application_id: int, guild_id: int, payload: list[dict[str, Any]]):
        r = Route(
            'PUT', '/{application_id}/guilds/{guild_id}/commands',
